...
export class BpmnVisualization {
  ...
  // Função para criar gateways com símbolos BPMN corretos
  createGateway(id, x, y, type, label) {
    ...
    switch (type) {
      case "exclusive":
        // Gateway exclusivo - X
        symbol = `<path d="M ${x - 8},${y - 8} L ${x + 8},${y + 8} M ${x + 8},${
          y - 8
        } L ${x - 8},${y + 8}" 
      ...
      case "parallel":
        // Gateway paralelo - +
        symbol = `<path d="M ${x - 8},${y} L ${x + 8},${y} M ${x},${
          y - 8
        } L ${x},${y + 8}" 
      ...
      case "inclusive":
        // Gateway inclusivo - O
        symbol = `<circle cx="${x}" cy="${y}" r="6" stroke="${strokeColor}" 
                         stroke-width="2" fill="none"/>`;
        break;
      case "event":
        // Gateway baseado em evento - pentagono duplo
        symbol = `<polygon points="${x - 6},${y - 4} ${x - 2},${y - 8} ${
          x + 2
        },${y - 8} ${x + 6},${y - 4} 
                             ${x + 6},${y + 4} ${x + 2},${y + 8} ${x - 2},${
          y + 8
                  ...
                  <polygon points="${x - 4},${y - 2} ${x - 1},${y - 6} ${
          x + 1
        },${y - 6} ${x + 4},${y - 2} 
                             ${x + 4},${y + 2} ${x + 1},${y + 6} ${x - 1},${
          y + 6
        ...
        break;
      default:
        // Gateway simples sem símbolo
        symbol = "";
    }

    return `
        <g data-element-id="${id}" class="bpmn-gateway">
          <polygon points="${x - 15},${y - 10} ${x + 15},${y} ${x - 15},${
      y + 10
    } ${x - 45},${y}" 
  ...
  generateSimpleDiagram() {
    return `
      <svg width="600" height="300" viewBox="0 0 600 300">
        ...
        <!-- Gateway de Decisão -->
        ${this.createGateway("Gateway_1", 300, 150, "exclusive", "Aprovado?")}
  ...
  generateComprasDiagram() {
    return `
      <svg width="900" height="400" viewBox="0 0 900 400">
        ...
        <!-- Gateway Final -->
        ${this.createGateway("Gateway_2", 690, 200, "exclusive", "Aprovado?")}
